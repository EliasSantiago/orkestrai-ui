...
import { customChatService } from '@/services/customChat';
...
export interface ConversationLifecycleAction {
  ...
  /**
   * Send message using custom backend
   */
  sendMessageWithCustomBackend: (params: {
...
> = (set, get) => ({
  sendMessage: async ({ message, files, onlyAddUserMessage }) => {
    ...
    // Check if we should use custom backend for this session
    if (enableCustomAuth) {
      const sessionStore = getSessionStoreState();
      const backendAgentId = sessionStore.getBackendAgentId(activeId);
  ...
  sendMessageWithCustomBackend: async ({ message, fileIdList, backendAgentId }) => {
    ...
    try {
      ...
      console.log('[CustomChat] Sending message to backend agent:', backendAgentId);
      const response = await customChatService.sendMessage({
        agentId: backendAgentId,
        content: message,
        sessionId: activeTopicId || activeId,
      });

      // 4. Update assistant message with response
      await messageService.updateMessage(assistantMessage.id, { content: response.content });

      console.log('[CustomChat] Response received:', response.content.substring(0, 100) + '...');

      // 5. Refresh session to update last activity
      getSessionStoreState().triggerSessionUpdate(activeId);
    } catch (error) {
      console.error('[CustomChat] Error sending message:', error);
