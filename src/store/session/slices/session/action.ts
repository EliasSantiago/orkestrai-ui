...
> = (set, get) => ({
  ...
  createSession: async (agent, isSwitchSession = true) => {
    const { switchSession, refreshSessions, syncAgentToBackend } = get();
    ...
    // Sync the newly created agent to the backend
    // Get the full session object after creation
    const createdSession = sessionSelectors.getSessionById(id)(get());
    if (createdSession && createdSession.type === LobeSessionType.Agent) {
      // Fire and forget - don't block on backend sync
      syncAgentToBackend(createdSession as LobeAgentSession).catch((error) => {
        console.error('[Session] Failed to sync agent to backend:', error);
      });
    }
